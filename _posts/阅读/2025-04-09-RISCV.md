...
|cmd_tpye | description | format | 
|:-----------------------------|:-----------------|--------:|
| SEW(Selected Element Width) | 指定向量元素运算的位宽 | 由配置指令(vsetivli)中的vtype域设置 | 
| LMUL (Vector Register Group Multiplier) | 可以拆分寄存器或者合并多个寄存器 | 由配置指令中的vtype域设置 | 
| VL (Vector Length) | 进行向量运算的元素数目，小于等于VLMAX （VLEN*LMUL/SEW） | 由配置指令中的vl域设置 |

vtype=cat(vediv[1:0],vsew[2:0],vlmul[1:0])，vediv[1:0] encodes how vector elements are divided into equal sub-elements
